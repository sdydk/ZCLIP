...
# from .hyperbolicity import mean_hyperbolicity_per_batch
...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor, **kwargs):
        ...
        )[0]  # 这里的self.attn应该有两返回值，一个是x，一个是attn_weight（对应于WeCLIP中的）  需要看一下attn_weight的维度
...
class Transformer(nn.Module):
    def __init__(
        self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, encode_type = "text_encoder"
    ):
        super().__init__()
        self.width = width
        self.layers = layers
        ...
        # # 2026-01-08 对文本特征进行双曲表征  仅对self.curv进行参数更新24G显存就不够，明天测试一下32G的多卡训练
        # self.curv = nn.Parameter(torch.tensor(0.1).log()) # curv_init=1.0    
        # self.textual_alpha = nn.Parameter(torch.tensor(512**-0.5).log())
        # text_feats = x.permute(1, 0, 2) * self.textual_alpha.exp()
                # with torch.autocast(x.device.type, dtype=torch.float32):
                #     text_feats = exp_map0(text_feats, self.curv.exp())
                # text_layer_features.append(text_feats)
    
    def forward(self, x: torch.Tensor, **kwargs):
        layer_features = [] # 
        for block in self.resblocks:
            x = block(x, **kwargs)
            layer_features.append(x.permute(1, 0, 2))
...
class VisionTransformer(nn.Module):
    ...
    def forward(
        ...
        x = self.conv1(x)  # shape = [*, width, grid, grid] [12, 3, 512, 512]--->[12, 768, 32, 32]
        b, _, gh, gw = x.size()
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]  [12, 1024, 768]
        x = torch.cat(
            [
            ...
            dim=1,
        )  # shape = [*, grid ** 2 + 1, width]   这是拼接的类别token，我在使用的时候记得只留空间维度[:, 1:, :]
        positional_embedding = self.positional_embedding
        if not (self.positional_embedding.shape[0] == x.shape[0]):
            cls_pos = positional_embedding[0:1, :]
            ...
            positional_embedding = torch.cat([cls_pos, per_pos_embedding])
        x = x + positional_embedding.to(x.dtype) # 位置编码操作
        x = self.ln_pre(x) # [12, 1025, 768]
        ...
        x, layer_features = self.transformer(x, key_padding_mask=mask) # x [1025, 12, 768]---> [12, 1025, 768]
        x = x.permute(1, 0, 2)  # LND -> NLD [1025, 12, 768]---> [12, 1025, 768]
        if return_cls:
            x = self.ln_post(x[:, 0, :]) # x表示的是分类tokens
        else:
            x = self.ln_post(x[:, 1:, :]) # x表示的是空间tokens
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, **kwargs):
        x, vision_layer_features =self.visual(image.type(self.dtype), **kwargs)
        return x, vision_layer_features
    ...
    def encode_text(self, text):
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND
        x, layer_features = self.transformer(x) # 
        
        x = x.permute(1, 0, 2)  # LND -> NLD
        x = self.ln_final(x).type(self.dtype)
        
        # x.shape = [batch_size, n_ctx, transformer.width] [50, 77, 512]
        # take features from the eot embedding (eot_token is the highest number in each sequence)
        x = x[torch.arange(x.shape[0]), text.argmax(dim=-1)] @ self.text_projection
        for i in range(len(layer_features)): # 
            fetures = layer_features[i]
            layer_features[i] = fetures[torch.arange(fetures.shape[0]), text.argmax(dim=-1)] @ self.text_projection

        return x, torch.stack(layer_features)
...
def build_model(state_dict: dict):
    ...
    model.load_state_dict(state_dict, strict=False) # 新添加了些层，初始化时需要strict=False
    return model.eval()
