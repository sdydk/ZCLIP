...
class PredefinedPromptExtractor(PromptExtractor):
    ...
    def forward(self, noun_list: List[str], clip_model: nn.Module):
        ...
        for template in self.templates:
            ...
            text_features, text_layer_features = clip_model.encode_text(text_inputs) # 
            text_features /= text_features.norm(dim=-1, keepdim=True)
            text_features_bucket.append(text_features)
            text_layer_features_bucket.append(text_layer_features) # [80, 12, 50, 512]
        del text_inputs
        # ensemble by averaging
        text_features = torch.stack(text_features_bucket).mean(dim=0)
        text_features = text_features / text_features.norm(dim=-1, keepdim=True)
        
        text_layer_features = torch.stack(text_layer_features_bucket).mean(dim=0) #
        
        return text_features, text_layer_features
